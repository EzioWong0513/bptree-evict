#ifndef _BPTREE_MIRA_PAGE_CACHE_LATENCY_H_
#define _BPTREE_MIRA_PAGE_CACHE_LATENCY_H_
...
/**
 * LatencyLevel enum for simulating different far memory latencies
 */
enum class LatencyLevel {
    NONE,       // No added latency
    LOW,        // 100 microseconds
    MEDIUM,     // 500 microseconds
    HIGH        // 1 millisecond
};

/**
 * LatencyCacheSection represents a portion of the cache dedicated to
 * a specific access pattern, mimicking Mira's section separation
 */
class LatencyCacheSection {
public:
    LatencyCacheSection(std::string name, AccessPattern pattern, size_t size, size_t line_size)
...
/**
 * MiraLatencyPageCache implements a Mira-inspired page cache with
 * multiple sections for different access patterns and
 * program-guided eviction hints, with simulated far memory latency
 */
class MiraLatencyPageCache : public AbstractPageCache {
public:
    MiraLatencyPageCache(size_t page_size, LatencyLevel latency = LatencyLevel::NONE, 
                         size_t total_cache_size = 4 * 1024 * 1024)
        : page_size(page_size), latency(latency), total_cache_size(total_cache_size)
    {
        next_id.store(1);
        ...
        default_section = std::make_unique<LatencyCacheSection>("default", AccessPattern::UNKNOWN, 
                                                        total_cache_size, page_size);
                                                        
        std::cout << "MiraLatencyPageCache initialized with " << sections.size() 
                  << " sections and a default section" << std::endl;
        std::cout << "Far memory latency simulation: " << get_latency_name() << std::endl;
    }

    // Set the simulated far memory latency
    void set_latency(LatencyLevel new_latency) {
        latency = new_latency;
        std::cout << "Far memory latency changed to: " << get_latency_name() << std::endl;
    }

    // Get the current latency name as string
    std::string get_latency_name() const {
        switch (latency) {
            case LatencyLevel::NONE: return "None";
            case LatencyLevel::LOW: return "Low (100μs)";
            case LatencyLevel::MEDIUM: return "Medium (500μs)";
            case LatencyLevel::HIGH: return "High (1ms)";
    ...
    virtual Page* fetch_page(PageID id, boost::upgrade_lock<Page>& lock) override
    {
        ...
        if (it == page_map.end()) {
            // Page not in cache - simulate far memory access latency
    ...
    {
        // Only process every Nth page to reduce overhead (optimization)
        if (id % 5 != 0) return;
...
private:
    ...
    std::vector<LatencyCacheSection> sections;
    std::unique_ptr<LatencyCacheSection> default_section;
    ...
    void update_lru(PageID id) {
        // Fast path check without taking heavier locks (optimization)
        auto section_it = page_section_map.find(id);
        if (section_it == page_section_map.end()) return;
        
        std::lock_guard<std::mutex> guard(section_mutex);
        int section_idx = section_it->second;
        
        if (section_idx >= 0 && section_idx < static_cast<int>(sections.size())) {
            sections[section_idx].touch_page(id);
        } else if (section_idx == -1) {
            default_section->touch_page(id);
        }
...
// Also create a memory page cache with simulated latency for fair comparison
class MemLatencyPageCache : public AbstractPageCache {
public:
    MemLatencyPageCache(size_t page_size, LatencyLevel latency = LatencyLevel::NONE) 
        : page_size(page_size), latency(latency) 
    { 
        next_id.store(1); 
        std::cout << "MemLatencyPageCache initialized with far memory latency: " 
                  << get_latency_name() << std::endl;
    }
