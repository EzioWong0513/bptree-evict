...
all: $(TARGET) $(MIRA_TARGET) $(BENCHMARK_TARGET) $(LATENCY_TARGET)
...
clean:
	rm -f $(TARGET) $(MIRA_TARGET) $(BENCHMARK_TARGET) $(LATENCY_TARGET) $(OBJS) $(MIRA_OBJS) $(BENCHMARK_OBJS) $(LATENCY_OBJS)
	rm -rf tmp/*
	rm -f profile.pdf profile.svg profile.prof
	rm -f *.txt

.PHONY: all clean tests mira_tests benchmark latency latency_quick latency_thorough